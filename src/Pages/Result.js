...
const Result = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const scoresCollectionRef = collection(database, 'userscores');
      unsubscribeScores = onSnapshot(scoresCollectionRef, (snapshot) => { // Use onSnapshot
    ...
    const authenticateUser = () => {
      unsubscribeAuth = auth.onAuthStateChanged((userData) => {
        if (!userData?.email) {
          onOpen();
          setTimeout(() => {
            navigate('/');
    ...
    return () => {
      ...
      unsubscribeScores && unsubscribeScores();
    };
  }, [navigate, onOpen]);
